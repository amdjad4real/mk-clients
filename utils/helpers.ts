...
/**
 * Compresses an image to be under a target size (default 200KB)
 */
export const compressImage = async (file: File, targetSizeKB: number = 200): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // Iteratively reduce quality if still over target size
        // 1 byte = 0.001 KB, but base64 is ~33% larger than binary
