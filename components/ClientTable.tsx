...
import { Search, RefreshCcw, Edit, Copy, Trash2, User, Check, CreditCard, Calendar as CalendarIcon, Tag, Clock, Plane, CreditCard as CardIcon, Database, CheckSquare, Square, CheckCircle, ShieldAlert } from 'lucide-react';
...
const ClientTable: React.FC<ClientTableProps> = ({ 
  ...
  const handleCopyAction = (client: Client) => {
    const raw = `Name: ${client.lastName} ${client.firstName}\nPassport: ${client.passportNumber}\nDOB: ${client.dob}\nCat: ${client.category}`;
    navigator.clipboard.writeText(raw).then(() => { setCopiedId(client.id); setTimeout(() => setCopiedId(null), 2000); });
  };
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 p-5 flex flex-col md:flex-row gap-5">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder={t.search} value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-12 pr-4 rtl:pr-12 rtl:pl-4 py-3.5 rounded-2xl border-2 border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none font-bold transition-all" />
        </div>
        <div className="flex gap-3">
          <div className="relative">
            <CalendarIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <input type="date" value={dateFilter} onChange={(e) => setDateFilter(e.target.value)} className="pl-12 pr-6 py-3.5 rounded-2xl border-2 border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-4 focus:ring-blue-500/10 outline-none text-xs font-black uppercase" />
          </div>
          <button onClick={() => { setSearch(''); setDateFilter(''); }} className="p-3.5 rounded-2xl bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 transition-all active:scale-95 shadow-sm"><RefreshCcw className="w-5 h-5 text-slate-600 dark:text-slate-400" /></button>
        </div>
      </div>
          ...
          sortedGroupKeys.map(dateKey => (
            <div key={dateKey} className="space-y-4">
              <div className="flex items-center gap-6 px-4">
                ...
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.3em] flex items-center gap-3"><Tag className="w-4 h-4 text-indigo-500" />{formatDateLabel(dateKey)}<span className="bg-indigo-600 text-white px-3 py-1 rounded-xl text-[10px] shadow-lg">{groupedClients[dateKey].length} {t.files}</span></h3>
                <div className="h-px flex-1 bg-gradient-to-r from-transparent via-slate-300 dark:via-slate-700 to-transparent" />
              </div>
                  ...
                  <table className="w-full text-left rtl:text-right">
                    <thead>
                      <tr className="bg-slate-50/50 dark:bg-slate-900/50 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] border-b dark:border-slate-700/50">
                        <th className="px-6 py-5 text-center w-12"><button onClick={handleToggleSelectAll} className="hover:text-indigo-500 transition-colors">{filteredVisibleClients.length > 0 && filteredVisibleClients.every(c => selectedClientIds.includes(c.id)) ? <CheckSquare className="w-5 h-5" /> : <Square className="w-5 h-5" />}</button></th>
                      ...
                      {groupedClients[dateKey].map((client) => {
                        ...
                        return (
                          <tr key={client.id} className={`group transition-all border-l-[8px] ${isSelected ? 'bg-indigo-50/40 dark:bg-indigo-900/20 border-indigo-600' : styles.row} ${isModified && !isSelected ? 'bg-red-50/20 dark:bg-red-950/20 animate-pulse' : ''}`}>
                            <td className="px-6 py-5 text-center"><button onClick={() => onToggleClientSelect?.(client.id)} className={`${isSelected ? 'text-indigo-600' : 'text-slate-300 dark:text-slate-600'} hover:text-indigo-500 transition-all`}>{isSelected ? <CheckSquare className="w-5 h-5" /> : <Square className="w-5 h-5" />}</button></td>
                            <td className="px-6 py-5"><div className="w-14 h-14 rounded-2xl bg-white dark:bg-slate-900 overflow-hidden flex items-center justify-center border-2 border-slate-100 dark:border-slate-700 shadow-xl group-hover:scale-110 transition-transform">{client.photoUrl ? <img src={client.photoUrl} alt="" className="w-full h-full object-cover" /> : <User className="w-7 h-7 text-slate-300" />}</div></td>
                                    ...
                                    <div className="flex flex-col items-start gap-1">
                                      <div className="flex items-center gap-1.5 px-2 py-0.5 bg-red-600 text-white text-[8px] font-black rounded-lg shadow-lg shadow-red-500/30">
                                        ...
                                        {onConfirmModification && (
                                          <button onClick={(e) => { e.stopPropagation(); onConfirmModification(client.id); }} className="ml-1 p-0.5 bg-white/20 rounded hover:bg-white/40 transition-colors" title="Confirm modification"><CheckCircle className="w-2.5 h-2.5" /></button>
                                        )}
                                      </div>
                                      <span className="text-[7px] font-black text-red-500/70 uppercase leading-none">
                                        {new Date(client.updatedAt).toLocaleDateString()} @ {new Date(client.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                      </span>
                                    </div>
                                  )}
                                </div>
                                <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 mt-1"><Clock className="w-3 h-3" /> {new Date(client.updatedAt || client.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                              </div>
                            </td>
                            ...
                            <td className="px-6 py-5"><span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase border shadow-sm ${styles.badge}`}>{client.category}</span></td>
                            <td className="px-6 py-5"><div className="flex flex-col gap-1.5"><div className="flex items-center gap-2 text-[11px] font-black text-slate-900 dark:text-white"><Plane className="w-4 h-4 text-blue-500" /> {client.previousVisaNumber || '---'}</div><div className="text-[9px] font-bold text-slate-400">{client.visaFrom || '---'} &rarr; {client.visaTo || '---'}</div></div></td>
                            <td className="px-6 py-5"><div className="flex flex-col gap-1.5"><div className="flex items-center gap-2 text-[11px] font-black text-slate-900 dark:text-white"><CardIcon className="w-4 h-4 text-amber-500" /> {client.payment.cardMask || '---'}</div><div className="text-[9px] font-bold text-slate-400">EXP: {client.payment.expiryDate || '--'} | CVV: {client.payment.cvv ? '***' : '--'}</div></div></td>
                            <td className="px-6 py-5">
                              <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                <button onClick={() => onEdit(client)} className="p-2.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-sm"><Edit className="w-4 h-4" /></button>
                                <button onClick={() => handleCopyAction(client)} className={`p-2.5 rounded-xl transition-all shadow-sm ${copiedId === client.id ? 'bg-emerald-600 text-white' : 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 hover:bg-emerald-600 hover:text-white'}`}>{copiedId === client.id ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}</button>
                                <button onClick={() => onDelete(client.id)} className="p-2.5 bg-red-50 dark:bg-red-900/30 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm"><Trash2 className="w-4 h-4" /></button>
                              </div>
                            </td>
                          </tr>
                        );
